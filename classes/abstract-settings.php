...
abstract class Abstract_Settings {
	...
	public function add_plugin_action_links( $actions ) {
		$settings_link_name = __( 'Settings', 'kntnt-auto-excerpt-generator' );
	...
	public function options_page() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'Unauthorized use.', 'kntnt-auto-excerpt-generator' ) );
		}
	...
	private function render_settings_page() {
		...
		if ( $unsatisfied_dependencies = Plugin::unsatisfied_dependencies() ) {
			$message = sprintf( _n( 'This plugin must be installed and active: %s', 'These plugins must be installed and active: %s', count( $unsatisfied_dependencies ), 'kntnt-auto-excerpt-generator' ), join( ', ', $unsatisfied_dependencies ) );
	...
	private function update_options( $opt ) {
		...
		if ( ! wp_verify_nonce( $_POST['_wpnonce'], $this->ns ) ) {
			wp_die( __( 'Nonce failed.', 'kntnt-auto-excerpt-generator' ) );
		}
	...
	private function notify_error( $field ) {
		if ( isset( $field['validate-error-message'] ) ) {
			...
			$message = sprintf( __( '<strong>ERROR:</strong> Invalid data in the field <em>%s</em>.', 'kntnt-auto-excerpt-generator' ), $field['label'] );
		}
		else {
			$message = __( '<strong>ERROR:</strong> Please review the settings and try again.', 'kntnt-auto-excerpt-generator' );
		}
		$this->notify_admin( $message, 'error' );
	}

	private function notify_success() {
		$message = __( 'Successfully saved settings.', 'kntnt-auto-excerpt-generator' );
